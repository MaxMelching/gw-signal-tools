    ...
    for param_val in param_vals:
        ...
        if distance_kind == 'diff_norm':
            if not use_opt_dist:
                ...
                # -- Optimization is wanted, have to rewrite for this to
                # -- take action.
                norm_wf = norm(wf, **inner_prod_kwargs)
                wf_overlap = inner_product(wf, center_wf, **inner_prod_kwargs)
                distance_val = np.sqrt(
                    np.abs(norm_wf**2 - 2 * wf_overlap + norm_center_wf**2)
                )
                # -- Numerical errors can make it smaller than zero,
                # -- thus taking absolute value here is needed.
    ...
    **inner_prod_kwargs,
    ...
    if not isinstance(param_to_vary, str):
        if len(param_to_vary) == 1:
            ...
            non_proj_params = [
                val for val in param_to_vary if val not in params_to_project
            ]
    ...
    if params_to_project is not None:
        non_proj_indices = [
            i for i, val in enumerate(param_to_vary) if val not in params_to_project
        ]
