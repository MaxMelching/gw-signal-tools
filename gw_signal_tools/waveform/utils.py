...
__all__ = ('td_to_fd_waveform', 'fd_to_td_waveform', 'pad_to_get_target_df',
           'restrict_f_range', 'fill_f_range', 'get_signal_at_target_df',
           'get_signal_at_target_frequs', 'get_strain', 'get_wf_generator', )
...
def fd_to_td_waveform(signal: FrequencySeries) -> TimeSeries:
    ...
    the function :code:`~gw_signal_tools.waveform_utils.
    pad_to_get_target_df`. The time span to be padded should be roughly
    the same as the epoch (i.e. the starting time) of the signal, so
    that a shift by it does not cause a wrap-around.
    ...
    # More handling of wrap-around of signal (in case epoch is zero, which
    # means none has been set, the gwsignal default)
    if signal.epoch.value == 0.0:
        number_to_roll = out.size * 7 // 8
        # number_to_roll = out.size // 2  # Which one is better?

        out = np.roll(out, shift=number_to_roll)
        out.times -= out.times[number_to_roll]

        # Perhaps a fancy estimator like PyCBC has would be nice
    # TODO: could also build in elif case with check between duration and epoch
    # -> if duration is not longer than twice the epoch, we know that wrap-around is likely to occur
...
def restrict_f_range(
    ...
    f_fill_lower = _q_convert(fill_range[0] if fill_range[0] is not None else f_lower,
                              frequ_unit, 'f_fill_lower', 'signal.frequencies')
    f_fill_upper = _q_convert(fill_range[1] if fill_range[1] is not None else f_upper,
                              frequ_unit, 'f_fill_upper', 'signal.frequencies')
    ...
    if (copy and (lower_number == 0)
        and (f_fill_lower != f_lower) and (f_fill_upper != f_upper)):
...
def fill_f_range(
    ...
    if (f_fill_lower := fill_bounds[0]) is not None:
        f_fill_lower = _q_convert(f_fill_lower, frequ_unit, 'f_fill_lower',
                                  'signal.frequencies')        
        if f_fill_lower < f_lower:
            logger.info(
    ...
    if (f_fill_upper := fill_bounds[1]) is not None:
        f_fill_upper = _q_convert(f_fill_upper, frequ_unit, 'f_fill_upper',
                                  'signal.frequencies')
        if f_fill_upper > f_upper:
            logger.info(
...
def get_signal_at_target_frequs(
    ...
    """
    target_frequencies = _q_convert(target_frequencies, signal.frequencies.unit,
                                    'target_frequencies', 'signal.frequencies')
