...
def fd_to_td_waveform(signal: FrequencySeries) -> TimeSeries:
    ...
    there are two options: either interpolating `signal` to a higher
    resolution in frequency space or, if `signal` itself is obtained
    from a Fourier transform, pad the time domain signal with zeros at
    its end (if the signal goes to zero at its end), e.g. using the
    function `~gw_signal_tools.waveform_utils.pad_to_get_target_df`.
    The time span to be padded should be roughly the same as the epoch
    (i.e. the starting time) of the signal, so that a shift by it does
    not cause a wrap-around.
...
def fill_f_range(
    ...
    input is changed. For a function with similar functionality that
    is not applied inplace, consider using `~gw_signal_tools.
    waveform_utils.restrict_f_range`.
