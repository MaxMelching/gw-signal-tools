...
def test_distance(param_to_vary, optimize, dist_kind):
    ...
    assert_allclose_quantity(
        u.Quantity([dist.xindex[0], dist.xindex[-1]]),
        param_range,
        atol=step_size.value,
        rtol=0.0,
    )
...
def test_projected_linearized_distance(param_to_vary):
    ...
    assert_allclose_quantity(
        u.Quantity([dist1.xindex[0], dist1.xindex[-1]]),
        param_range,
        atol=step_size.value,
        rtol=0.0,
    )
    ...
    assert_allclose_quantity(
        u.Quantity([dist2.xindex[0], dist2.xindex[-1]]),
        param_range,
        atol=step_size.value,
        rtol=0.0,
    )
...
class ErrorRaising(unittest.TestCase):
    ...
    def test_invalid_center_val(self):
        wrong_param_range = u.Quantity([0.5 * self.center_val, 0.9 * self.center_val])
